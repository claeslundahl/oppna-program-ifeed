...
public class IFeedServiceImpl implements IFeedService, Serializable {
  ...
  public IFeed getIFeed(final Long id) {
    ...
    IFeed result = objectRepo.findByPrimaryKey(IFeed.class, id);
      ...
      if (!filters.isEmpty()) {
        result.getFilters().addAll(filters.iterator().next().getFilters());
      }

      Collection<IFeed> composites =
          iFeedRepo .findByQuery(
              "SELECT distinct ifeed " +
                  "FROM IFeed ifeed " +
                  // "left join fetch ifeed.filters f "
      ...
      if (!composites.isEmpty()) {
        result.getComposites().addAll(composites.iterator().next().getComposites());
      }
      */
    ...
    init(result);
      ...
      for (DynamicTableDef dynamicTableDef : result.getDynamicTableDefs()) {
        dynamicTableDef.getExtraSorting().toString();
        dynamicTableDef.getColumnDefs().toString();
      }
  ...
  private void init(IFeed result) {
    try {
      ...
      System.out.println("Problem with result.id = " + result.getId());
      throw new RuntimeException(e);
    }
  }
