...
public class SolrQueryBuilder {
    ...
    public static String createQuery(IFeedFilter iFeedFilter) {
        ...
        switch (filter.getMetadataType()) {
            case TEXT_FREE:
                query  = filter.getFilterField() + ":" + iFeedFilter.getFilterQuery() + "*";
                break;
            case TEXT_FIX:
                query = filter.getFilterField() + ":" + iFeedFilter.getFilterQuery();
                break;
            case DATE:
                if(filter.name().contains("FROM_DATE")) {
                    query = filter.getFilterField() + ":[" + iFeedFilter.getFilterQuery() + " TO *]";
                } else if(filter.name().contains("TO_DATE")) {
                    query = filter.getFilterField() + ":[* TO " + iFeedFilter.getFilterQuery() + "]";
                } else {
