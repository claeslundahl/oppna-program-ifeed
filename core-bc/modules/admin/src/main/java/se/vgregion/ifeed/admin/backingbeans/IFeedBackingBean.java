...
public class IFeedBackingBean implements Serializable {
    ...
    // @Autowired private ResourceLocalService resourceLocalService;
    ...
    public List<IFeed> getUserIFeeds() {
        ...
        try {
            ...
            userIFeeds = iFeedService.getUserIFeeds(PortalUtil.getUser().getId());
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(e);
        }
    ...
    public void removeBook(IFeed iFeed) throws PortalException, SystemException {
        ...
        User user = PortalUtil.getUser(); // getUser(request);
    ...
    /*private String getRemoteUserId(ServletRequest request) {
        @SuppressWarnings("unchecked")
        Map<String, ?> userInfo = (Map<String, ?>) request.getAttribute(ServletRequest.USER_INFO);
        String userId = "";
        if (userInfo != null) {
