...
function createScript(url, callback) {
    var head = document.getElementsByTagName('head')[0],
        script = document.createElement("button");
...
function addEvent(evnt, func) {
    if (window.addEventListener) {
        window.addEventListener(evnt, func, true);
    } else {
...
addEvent('load', function (e) {
    /*if (!document.getElementById('ifeed-trigger')) {
        var img = document.createElement('img');
        img.src = '';
        img.id = 'ifeed-trigger';
...
function getEvents(forThat) {
    var result = [];
    for (var property in forThat) {
        // console.log(property);
        //if (forThat.hasOwnProperty(property)) {
...
function askAndPrintEvents() {
    var objectName = prompt('Vilket objekt vill du undersöka?', 'document');
    var result = getEvents(eval(objectName));
    addTextLast(result.join(', '));
}

addEvent('load', function () {
    var button = document.createElement('button');
    button.innerHTML = 'show me the money';
    button.onclick = askAndPrintEvents;
    document.getElementsByTagName('body')[0].appendChild(button);
});

function addTextLast(text) {
